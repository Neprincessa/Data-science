select CountryRegion, StateProvince, City,
    ...
    abs((count([SalesLT].[Customer].CustomerID) - lag(count([SalesLT].[Customer].CustomerID)) over (partition by CountryRegion order by count(Customer.CustomerID) desc, City))) as Difference,
